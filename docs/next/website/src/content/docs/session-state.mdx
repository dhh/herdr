...
options that switch the agent to a non-interactive run (`--print`, `--prompt`) are never
replayed, and neither is a prompt typed on the launch command line. An option Herdr does not
recognize is replayed when it stands alone and dropped when it is followed by a bare value,
because replaying it without its value would break the resume command.
