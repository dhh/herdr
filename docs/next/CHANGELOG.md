...
- Native agent session restore now replays the options the agent was started with, so a pane started as `claude --permission-mode bypassPermissions` resumes with that permission mode instead of default permissions. Session-selection options, one-shot options, and launch prompts are not replayed.
